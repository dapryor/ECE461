...
void Init_Comparator(void) {
	...
	SIM->SCGC4 |= (1UL << SIM_SCGC4_CMP_SHIFT); 
	
	// Enable Comparator
	CMP0->CR1 = (1 << CMP_CR1_EN_SHIFT);

	// Select input channels:
	// Plus: channel 5 on Port E bit 29
	// Minus: CMP DAC is channel 7
	CMP0->MUXCR = (5 << CMP_MUXCR_PSEL_SHIFT) | 
								(7 << CMP_MUXCR_MSEL_SHIFT);

	// Set reference voltage at 32/64 = 1/2 VREFH
	CMP0->DACCR = (1 << CMP_DACCR_DACEN_SHIFT ) |
								(32 << CMP_DACCR_VOSEL_SHIFT);
	
	
	// Enable interrupt for Comparator on both edges
	CMP0->SCR = (1 << CMP_SCR_IEF_SHIFT) |
							(1 << CMP_SCR_IER_SHIFT);
